...
export default {
  ...
  data () {
    return {
      ...
      confirmPassword: '',
      newUser: {},
      userList123: JSON.parse(window.localStorage.getItem('userList'))
    }
  },
  methods: {
    goLogin () {
      ...
      if (this.password !== this.confirmPassword) {
        ...
        this.$store.state.userList = this.tUserList
        this.$store.state.userList.push(this.newUser)
        window.localStorage.setItem('userList', JSON.stringify(this.$store.state.userList))
    ...
    goHome () {
      ...
      for (let index in users) {
        if (users[index].username === this.username && users[index].password === this.password) {
          this.login = false
          window.localStorage.setItem('loginStatus', 'login')
          this.$store.commit('getUser', this.username)
          window.localStorage.setItem('username', this.username)
